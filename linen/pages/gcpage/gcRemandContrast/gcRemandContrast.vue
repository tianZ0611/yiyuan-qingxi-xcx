<template>
	<view>
		...
		<view class="posfb80 disf">
			<button class="w130h44 marl10 bclan colorbai" @click="gogetstatistics()">确认送回</button>
			<button v-show="saomiao == false" class="w130h44 marl20 bclan colorbai"
				@click="inventoryStart()">开始扫描</button>
			<button v-show="saomiao == true" class="w130h44 marl20 bccheng colorbai"
				@click="inventoryStop()">关闭扫描</button>
			<view class="marl20 padl10 padr10 bcchong textjz colorbai linh44 borr8" @click="qing()">复原</view>
		</view>
	</view>
</template>
		...
		methods: {
			...
			gogetstatistics() { //点击下一步
				...
				if (that.saomiao == false && jiaoZhunOrder != false && that.duodeid.length < 1) { //检测未关闭扫描状态false为关闭
					...
					HuiShouJiLu(data).then(res => { //回收接口
