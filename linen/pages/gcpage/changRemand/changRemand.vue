	...
	<view class="box fs16">
		<view class="fs16 bcbai padl20 padt10 padb10 marb8 posr bor8" v-for="i in list" :key="i.id">
			...
			<view class="w80h34 textjz linh34 bclan colorbai borr8 posab10r20"
				@click="goqueren(i.orderNumber,i.materialType,i.totalNumber,i.deptName)">
				详情
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 	})
			// },
			goqueren(id, lei, num, bu) {
				var that = this
				if (that.id == '51') {
					uni.navigateTo({
						url: '/pages/gcpage/gcRemandContrast/gcRemandContrast?id=' + id + '&lei=' + lei + '&num=' +
							num + '&bu=' + bu
					})
				} else if (that.id == '5') {
					uni.navigateTo({
						url: '/pages/gcpage/yyRemandContrast/yyRemandContrast?id=' + id + '&lei=' + lei + '&num=' +
							num + '&bu=' + bu
					})
