<template>
	<view class="box fs16">
		<view class="fs16 bcbai padl20 padt10 padb10 marb8 posr bor8" v-for="i in list" :key="i.id">
			...
			<view class="mart5">时间：{{i.affirmTime}}</view>
			<view class="mart5">数量：<span class="colorlan fs20 fw">{{i.totalNumber}}</span></view>
			<view class="w80h34 textjz linh34 bclan colorbai borr8 posab10r20" v-if="i.status != '送净中'"
				@click="goqueren(i.orderNumber,i.materialType,i.totalNumber,i.status)">
				详情
			</view>
			<view class="w80h34 textjz linh34 bchui colorbai borr8 posab10r20" v-else @click="xiwan()">
				详情
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 	})
			// },
			goqueren(id, lei, num, zhuangtai) {
				uni.navigateTo({
					url: '/pages/gcpage/xiYiStep/xiYiStep?id=' + id + '&lei=' + lei + '&num=' + num +
						'&zhuangtai=' + zhuangtai
				})
			},
			xiwan() {
