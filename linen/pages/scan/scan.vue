<template>
	<view>
		<uni-table class="w690" border stripe emptyText="暂无更多数据">
			...
			<!-- 表格数据行 -->
			<uni-tr class="fs12" v-for="i in list">
				<uni-td align="center">{{i.id}}</uni-td>
				<uni-td align="center">{{lingqu}}</uni-td>
				<uni-td align="center">{{tit}}</uni-td>
				<uni-td align="center">{{i.count}}</uni-td>
			</uni-tr>
		</uni-table>
		<view class="posfb80 disf">
			<button class="w130h44 marl10 bclan colorbai" @click="gogetstatistics()">领用({{lingqu}})</button>
